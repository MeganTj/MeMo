...
def train(args):
    ...
    episode_height_rewards = deque(maxlen=10)
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            for info in infos:
                if 'episode' in info.keys():
                    ...
                    episode_height_rewards.append(info['episode']['hr'])
        ...
        height_reward_mean, height_reward_min, height_reward_max = np.mean(episode_height_rewards), np.min(episode_height_rewards), np.max(episode_height_rewards)
        ...
        fp_log.write('iterations: {}, mean(len): {:.1f}, min(len): {}, max(len): {}, mean(eval reward): {:.3f}, min(eval reward): {:.3f}, max(eval reward): {:.3f}, mean(reward): {:.3f}, min(reward): {:.3f}, max(reward): {:.3f}, mean(height reward): {:.3f}, min(height reward): {:.3f}, max(height reward): {:.3f}, mean(master penalty): {:.3f}, min(master penalty): {:.3f}, max(master penalty): {:.3f}, value_loss: {:.3f}, action_loss: {:.3f}\n'.format(
            total_num_steps, len_mean, len_min, len_max, eval_reward_mean, eval_reward_min, eval_reward_max, reward_mean, reward_min, reward_max, height_reward_mean, height_reward_min, height_reward_max, mpen_mean, mpen_min, mpen_max, value_loss, action_loss))
        fp_log.close()
        # logging to console
        if j % args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            print(
                "Updates {}, num timesteps {}, FPS {}, time {} minutes \n Last {} training episodes: mean/median length {:1f}/{}, min/max length {}/{} mean/median eval reward {:.1f}/{:.1f}, min/max eval reward {:.1f}/{:.1f}, mean/median reward {:.1f}/{:.1f}, min/max reward {:.1f}/{:.1f}, mean/median height reward {:.1f}/{:.1f}, min/max height reward {:.1f}/{:.1f}, mean/median master penalty {:.3f}/{:.3f}, min/max master penalty {:.3f}/{:.3f}\n"
                .format(j, total_num_steps,
                        ...
                        np.mean(episode_height_rewards), np.median(episode_height_rewards), 
                        np.min(episode_height_rewards), np.max(episode_height_rewards), 
