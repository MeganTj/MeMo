...
args.base_last_hidden_size = args.base_last_hidden_size if args.base_last_hidden_size != -1 else args.base_hidden_size
rank_str = f"rank_{args.robot_rank}"

# Organize runs by hyperparameter values
train_script = "scripts/train_rl.py"
str_hyperparam = f'ns={args.num_steps}_dw={args.dist_weight}_nw={args.norm_weight}_lr={args.lr}_lrs_{args.lr_schedule[:3]}_total-ns={args.num_env_steps}_np={args.num_processes}_nmb={args.num_mini_batch}_' + \
...
if args.train_mode != "rl":
    ...
    il_hyperparam = f"ep_{args.il_epoch}_loss_{args.loss_mode[:3]}_bs_{args.il_batch_size}_il-lr_{args.il_lr}_de_{args.dagger_epoch}_dag_{use_dagger}_imi_{args.il_model_idx}_hi_{args.hi_mode}_lstd_{args.logstd_mode[:3]}_els_{args.extend_local_state}_" +\
                f"nl_{args.nonlinearity_mode[:3]}_sfx_{hierarchy_suffix}_swm_{args.share_within_mod}_dhs_{args.decoder_hidden_size}_mhs_{args.module_hidden_size}_l1_{args.l1_weight}_noi_{args.use_noise}_nstd_{args.noise_levels}_nrel_{args.noise_relative}_" + \
                f"ntr_{args.noise_train}"
    save_dir = os.path.join(args.save_pref, args.env_name + f"-fs={args.frame_skip}", args.task, rank_str, f"rl_{str_hyperparam}", f"{args.train_mode}_{il_hyperparam}")
else:
    if args.load_model_path is not None:
        if args.transfer_str is not None:
            save_dir = os.path.join(args.save_pref, args.env_name + f"-fs={args.frame_skip}", args.task, rank_str, f'tr_{args.transfer_str}_hi_{args.hi_mode}_sfx_{hierarchy_suffix}_els_{args.extend_local_state}_swm_{args.share_within_mod}_lstd_{args.logstd_mode[:3]}_tm_{args.transfer_modules}_ft_{args.finetune_model}_lm_{args.load_master}_sg_{args.sqrt_gain}_sll_{args.set_low_logstd}_{str_hyperparam}')
        else:
            save_dir = os.path.join(os.path.dirname(os.path.dirname(args.load_model_path)), f'hi_{args.hi_mode}_els_{args.extend_local_state}_swm_{args.share_within_mod}_lstd_{args.logstd_mode[:3]}_tm_{args.transfer_modules}_ft_{args.finetune_model}_lm_{args.load_master}_sll_{args.set_low_logstd}_sml_{args.set_mod_logstd}_{str_hyperparam}')
            args.load_model_path = f'"{args.load_model_path}"'
    else:
        save_dir = os.path.join(args.save_pref, args.env_name + f"-fs={args.frame_skip}", args.task, rank_str, f'hi_{args.hi_mode}_noi_{args.use_noise}_nstd_{args.noise_levels}_sni_{args.sni_mode[:3]}_lstd_{args.logstd_mode[:3]}_els_{args.extend_local_state}_swm_{args.share_within_mod}_sfx_{hierarchy_suffix}_dhs_{args.decoder_hidden_size}_mhs_{args.module_hidden_size}_dhl_{args.decoder_num_hidden_layers}_{str_hyperparam}')
print(save_dir)
...
cmd = f'python {train_script} --env-name {args.env_name} --task {args.task} --hier-env-name {args.hier_env_name} ' + \
        ...
        f'--dist-weight {args.dist_weight}  ' + \
        f'--pos-dist-weight {args.pos_dist_weight}  ' + \
